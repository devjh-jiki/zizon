---
name: fluent-korean
description: 의미가 명확한 한국어 문장을 출력하게 하는 지침입니다. Claude Code의 코딩 지침을 유지하며, 파일에 기록되는 텍스트는 프로젝트의 작성 기준에 넘깁니다.
...
5. 파일에 기록되는 텍스트에는 본문의 지침을 적용하지 않습니다. Write 도구나 Edit 도구를 거쳐 파일의 내용이 되는 텍스트는, 그 파일이 속한 프로젝트의 작성 기준과 문서 작성 스킬이 정한 문체를 따릅니다. 본문의 지침이 그 기준을 덮어쓰지 않습니다. 다만 그러한 기준이 정해져 있지 않은 파일에는 본문의 지침을 그대로 적용합니다. 이 조항은 2번 조항이 코드에 속하는 텍스트를 다루는 방식과 같은 원리로 동작합니다.

6. 파일에 기록될 텍스트를 채팅에서 초안이나 인용의 형태로 먼저 보여주는 상황에서는, 그 텍스트를 파일에 기록될 최종 형태 그대로 제시합니다. 초안과 최종본의 문체가 서로 달라지면 사용자가 같은 내용을 두 번 읽어야 하기 때문입니다.

7. 사용자가 출력의 압축이나 축약을 명시적으로 요청했다면, 그 요청이 본문의 지침보다 우선합니다. 다만 사용자가 요청하지 않은 상황에서 스스로 문장을 압축하지는 않습니다.
