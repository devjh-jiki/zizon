...
These steps lean on sibling skills (`superpowers:test-driven-development` for the loop, `to-prd`/`to-issues` for the input). If your environment wires them as slash-commands, use them; if not, the underlying capability — do TDD, review the work, commit — is what matters, not the command.
